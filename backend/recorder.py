...
through recording training data for each motion. Saves CSVs to data/serial/.

Usage:
    python recorder.py                  # auto-detect port, start recording
    python recorder.py --port COM3      # specify serial port
...
RECORD_DIR = Path(__file__).parent.parent / "data" / "serial"
ALL_MOTIONS = [
...
def record_session(port: str, baud_rate: int = BAUD_RATE):
    ...
    training data for each motion, saved as CSV to data/serial/.
    ...
    # Check existing recordings to know where to resume
    existing_counts = {}
    for motion in ALL_MOTIONS:
        count = 0
        for i in range(1, 100):
            path = RECORD_DIR / f"{motion} {i}.csv"
            if path.exists():
                count = i
            else:
                break
        existing_counts[motion] = count

    total_saved = 0

    for motion in ALL_MOTIONS:
        already = existing_counts[motion]
        needed = RECORDINGS_PER_MOTION - already
        if needed <= 0:
            print(f"  [{motion}] Already have {already} recordings -- skipping")
            continue

        print(f"\n  {'=' * 50}")
        print(f"  MOTION: {motion.upper()}")
        print(f"  {already} existing, recording {needed} more")
        print(f"  {'=' * 50}")

        for rec_idx in range(already + 1, already + needed + 1):
            filename = f"{motion} {rec_idx}.csv"
            filepath = RECORD_DIR / filename

            print(f"\n  --- {filename} ({rec_idx}/{RECORDINGS_PER_MOTION}) ---")
    ...
    print(f"  Run 'python build_templates.py --source serial' to build templates")
    print(f"  {'=' * 50}")
