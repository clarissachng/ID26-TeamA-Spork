...
export function createTutorialDetail(): HTMLElement {
  ...
  const observer = new MutationObserver(() => {
    if (page.classList.contains('active')) {
      ...
      if (motion === 'grinding') {
        xyMap = new SensorXYMap(cupContainer, '/ID26-TeamA-Spork/assets/motion_arrows/circle.png');
        xyMap.startListening();
      } else if (motion === 'up_down') {
        zStrip = new SensorZStrip(cupContainer, '/ID26-TeamA-Spork/assets/motion_arrows/up_down.png');
        zStrip.startListening();
      } else if (motion === 'press_down') {
        zStrip = new SensorZStrip(cupContainer, '/ID26-TeamA-Spork/assets/motion_arrows/press_down.png');
        zStrip.startListening();
      } else {
